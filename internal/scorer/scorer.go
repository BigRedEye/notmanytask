...
func (s Scorer) scoreMergeRequest(
	...
	case mergeRequestStatusMerged:
		...
		// when its last pipeline was created. No pipeline evidence, no score.
	...
	case mergeRequestStatusOnReview:
		scored.Status = TaskStatusOnReview
	...
	case mergeRequestStatusPending:
		scored.Status = TaskStatusPending
		scored.Message = s.timeToMerge(mergeRequest)
...
// timeToMerge tells how long until the review period is over.
func (s Scorer) timeToMerge(mergeRequest *models.MergeRequest) string {
	...
	return last.Add(s.mergeRequests.ReviewTtl).Sub(s.now()).Round(time.Minute).String()
}
