...
var testNow = testDeadline.Add(24 * time.Hour)
...
func TestMergeRequestMerged(t *testing.T) {
	...
	// Merged with a non-successful pipeline gives no score
	mr := merged(1, testDeadline, testRobot)
	mr.LastPipelineStatus = models.PipelineStatusFailed
	if task := scoreMergeRequests(t, []models.MergeRequest{mr}); task.Score != 0 {
...
func TestMergeRequestSelection(t *testing.T) {
	...
	if task = scoreMergeRequests(t, []models.MergeRequest{handMerged}); task.Status != TaskStatusSuccess || task.Score != 0 {
