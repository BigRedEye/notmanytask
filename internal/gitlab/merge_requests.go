...
func (p *MergeRequestsFetcher) syncProject(log *zap.Logger, project *gitlab.Project) error {
	mergedTasks, err := p.db.ListMergedTasks(project.Name)
	if err != nil {
		return errors.Wrap(err, "Failed to list merged tasks")
	}
	...
	for {
		...
		for _, mr := range mergeRequests {
			...
			if mergedTasks[task] {
				// The task is done, the row is frozen
				continue
			}
...
func (p *MergeRequestsFetcher) updateProject(log *zap.Logger, project *gitlab.Project) error {
	...
	for {
		...
		for _, branch := range branches {
			...
			if err := p.updateBranch(log.With(lf.BranchName(branch.Name)), project, branch.Name, reviewDeadline); err != nil {
...
func (p *MergeRequestsFetcher) updateBranch(log *zap.Logger, project *gitlab.Project, branch string, reviewDeadline time.Time) error {
	mrs, err := p.loadBranchMergeRequests(project, branch)
	if err != nil {
		return err
	}
	...
	if mrs.Open == nil {
		log.Info("No merge request found, creating a new one")
